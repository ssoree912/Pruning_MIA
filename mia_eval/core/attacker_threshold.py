...
Ported to project root for direct import.
Original logic adapted from WeMeM-main/attacker_threshold.py
"""
import numpy as np
...
class ThresholdAttacker:
    ...
    def _mem_inf_benchmarks(self):
        confidence = self._mem_inf_thre('confidence', self.s_tr_conf, self.s_te_conf, self.t_tr_conf, self.t_te_conf)
        entropy = self._mem_inf_thre('entropy', -self.s_tr_entr, -self.s_te_entr, -self.t_tr_entr, -self.t_te_entr)
        modentr = self._mem_inf_thre('modified entropy', -self.s_tr_m_entr, -self.s_te_m_entr, -self.t_tr_m_entr,
                                     -self.t_te_m_entr)
        return confidence, entropy, modentr

    def _mem_inf_benchmarks_non_cls(self):
        confidence = self._mem_inf_thre_non_cls('confidence', self.s_tr_conf, self.s_te_conf, self.t_tr_conf, self.t_te_conf)
        entropy = self._mem_inf_thre_non_cls('entropy', -self.s_tr_entr, -self.s_te_entr, -self.t_tr_entr, -self.t_te_entr)
        modentr = self._mem_inf_thre_non_cls('modified entropy', -self.s_tr_m_entr, -self.s_te_m_entr, -self.t_tr_m_entr,
                                             -self.t_te_m_entr)
        return confidence, entropy, modentr
