...
Example:
  python mia_eval/runners/run_mia_multi_victims.py \
    ...
    --prune_method static --sparsity 0.9 \
...
def run_one(victim: int, shadows: list, args, sparsity: float) -> bool:
    ...
    # DPF freeze tag support
    if args.prune_method == 'dpf' and getattr(args, 'freeze_tag', None):
        cmd += ['--freeze_tag', str(args.freeze_tag)]
    ...
    print(f"\n▶️ Victim {victim} | Shadows {shadows} | sparsity={sparsity}")
    print('   $ ' + ' '.join(cmd))
    try:
        res = subprocess.run(cmd, check=True, cwd=str(REPO_ROOT))
    ...
    for sp in sparsity_list:
        ...
        for v in seeds:
            ...
            if not shadows:
                ...
                continue
            if run_one(v, shadows, args, sparsity=sp):
                ok += 1
            else:
                fail += 1
